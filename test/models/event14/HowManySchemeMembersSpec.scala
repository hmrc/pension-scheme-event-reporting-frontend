...
class HowManySchemeMembersSpec extends AnyFreeSpec with Matchers with ScalaCheckPropertyChecks with OptionValues {
  ...
  "HowManySchemeMembers" - {
    ...
    "must fail to deserialise invalid values" in {
      ...
      val gen = arbitrary[String] suchThat (!HowManySchemeMembers.values.map(_.toString).contains(_))

      forAll(gen) {
        invalidValue =>

          JsString(invalidValue).validate[HowManySchemeMembers] mustEqual JsError("error.invalid")
      }
    }
