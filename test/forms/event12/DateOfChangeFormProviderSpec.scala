...
class DateOfChangeFormProviderSpec extends SpecBase
  ...
  private val form = new DateOfChangeFormProvider().apply(min = stubMin, max = stubMax)
  ...
  "dateOfChange" - {
    ...
    behave like dateFieldYearNot4Digits(
      form = form,
      key = dateOfChangeKey,
      formError = FormError(dateOfChangeKey, "genericDate.error.invalid.year")
    )
