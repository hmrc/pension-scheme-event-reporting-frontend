...
class EmployerPayeReferenceFormProviderSpec extends StringFieldBehaviours with Constraints {
  ...
  ".value" - {
    ...
    "not bind value without a / as the fourth character" in {
      ...
      result.errors mustEqual Seq(FormError(fieldName, "employerPayeReference.event24.error.noSlash", ArraySeq(employerIdRefNoSlashRegex)))
    }

    "not bind value that contains a disallowed character" in {
      val result = form.bind(valueDetails("123/$abcDEF"))
      result.errors mustEqual Seq(FormError(fieldName, "employerPayeReference.event24.error.disallowedChars", ArraySeq(employerIdRefDisallowedCharsRegex)))
    }
