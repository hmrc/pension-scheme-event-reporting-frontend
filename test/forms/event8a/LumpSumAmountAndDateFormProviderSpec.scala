...
class LumpSumAmountAndDateFormProviderSpec extends SpecBase
  ...
  private val messageKeyLumpSumValueKey = "event8a.lumpSumAmountAndDate.value"
  ...
  "lumpSumDate" - {
    ...
    behave like mandatoryDateField(
      form = form,
      key = lumpSumDateKey,
      requiredAllKey = "event8a.lumpSumAmountAndDate.date.error.nothingEntered"
    )

    behave like dateFieldYearNot4Digits(
      form = form,
      key = lumpSumDateKey,
      formError = FormError(lumpSumDateKey, "event8a.lumpSumAmountAndDate.date.error.outsideDateRanges")
    )
    ...
    behave like dateFieldWithMin(
      ...
      key = lumpSumDateKey,
      min = stubMin,
      formError = FormError(lumpSumDateKey, messages("event8a.lumpSumAmountAndDate.date.error.outsideReportedYear",
        formatDateDMY(stubMin), formatDateDMY(stubMax)))
    )
