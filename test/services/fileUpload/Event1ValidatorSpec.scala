...
import services.fileUpload.Validator.monoidResult
...
//    }
    ...
    "must validate in an efficient amount of time" in {
      ...
      val parserResultFuture = CSVParser.split({
        new ByteArrayInputStream(source.getLines().mkString("\n").getBytes(java.nio.charset.StandardCharsets.UTF_8))
      }) {
        case row if rowNumber != 0 =>
          val result = validator.validateFields(rowNumber, row, 2022)
      ...
      val validationErrorsFuture = parserResultFuture.map { parserResult =>
        ...
        combinedResult.validated.map(_.foldLeft(ua)((acc, ci) => acc.setOrException(ci.jsPath, ci.value)))
      }

      Await.result(validationErrorsFuture, Duration.Inf)
