...
""" + payloadMain
    ...
    "return validation errors when present for the Payment Nature fields (Employer)" in {
      ...
      val csvFile = CSVParser.split(stringToInputStream(data)) { case (row, rowNumber) =>
        Option.when(rowNumber > 0) {
          validator.validateFields(rowNumber, row, TaxYear("2022"))
        }
      }
      val ua = UserAnswers().setOrException(TaxYearPage, TaxYear("2022"), nonEventTypeData = true)
...
object Event1ValidatorSpec {
  def stringToInputStream(str: String) = {
    new java.io.ByteArrayInputStream(str.getBytes(java.nio.charset.StandardCharsets.UTF_8.name))
  }
