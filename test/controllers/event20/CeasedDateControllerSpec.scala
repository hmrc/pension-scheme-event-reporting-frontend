...
import views.html.event20.CeasedDateView
...
class CeasedDateControllerSpec extends SpecBase with BeforeAndAfterEach with MockitoSugar {
  ...
  private val validAnswer = Event20Date(LocalDate.of(2023, 2, 12))
  ...
  "CeasedDate Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      val userAnswers = emptyUserAnswersWithTaxYear.setOrException(WhatChangePage, WhatChange.CeasedOccupationalScheme)
    ...
    "must save the answer and redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, postRoute).withFormUrlEncodedBody(ceasedDate(validAnswer.date): _*)
...
object CeasedDateControllerSpec {
  private val ceasedDateKey = "ceasedDate"
