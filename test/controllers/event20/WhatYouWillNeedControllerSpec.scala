...
import views.html.event20.WhatYouWillNeedView
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        ...
        contentAsString(result).removeAllNonces() mustEqual
          view(controllers.event20.routes.WhatChangeController.onPageLoad(waypoints).url)(request, messages(application)).toString
      }
    }
  }
}
