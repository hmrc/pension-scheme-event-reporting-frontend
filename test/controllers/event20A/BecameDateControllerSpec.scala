...
import controllers.event20A.BecameDateControllerSpec.becameDate
import forms.event20A.BecameDateFormProvider
...
import views.html.event20A.BecameDateView
...
class BecameDateControllerSpec extends SpecBase with BeforeAndAfterEach with MockitoSugar {
  ...
  private val validAnswer = LocalDate.of(2022, 7, 12)
  ...
  "BecameDate Controller" - {
    ...
    "must save the answer and redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, postRoute).withFormUrlEncodedBody(becameDate(validAnswer): _*)
...
object BecameDateControllerSpec {
  private val becameDateKey = "becameDateMasterTrust"
