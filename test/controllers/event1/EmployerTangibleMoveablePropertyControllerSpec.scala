...
class EmployerTangibleMoveablePropertyControllerSpec extends SpecBase with BeforeAndAfterEach with MockitoSugar {
  ...
  private def getRoute: String = routes.EmployerTangibleMoveablePropertyController.onPageLoad(waypoints, 0).url

  private def postRoute: String = routes.EmployerTangibleMoveablePropertyController.onSubmit(waypoints, 0).url
  ...
  "EmployerTangibleMoveableProperty Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers().set(EmployerTangibleMoveablePropertyPage(0), validValue).success.value
    ...
    "must save the answer and redirect to the next page when valid data is submitted (non empty value)" in {
      ...
      running(application) {
        ...
        val result = route(application, request).value
        val updatedAnswers = emptyUserAnswers.set(EmployerTangibleMoveablePropertyPage(0), validValue).success.value

        status(result) mustEqual SEE_OTHER
        redirectLocation(result).value mustEqual EmployerTangibleMoveablePropertyPage(0).navigate(waypoints, emptyUserAnswers, updatedAnswers).url
