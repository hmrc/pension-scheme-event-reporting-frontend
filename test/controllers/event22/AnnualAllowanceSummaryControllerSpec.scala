...
class AnnualAllowanceSummaryControllerSpec extends SpecBase with BeforeAndAfterEach with MockitoSugar {
  ...
  private def getRoute: String = routes.AnnualAllowanceSummaryController.onPageLoad(waypoints).url

  private def postRoute: String = routes.AnnualAllowanceSummaryController.onSubmit(waypoints).url
