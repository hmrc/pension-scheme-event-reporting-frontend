...
class HasSchemeChangedRulesControllerSpec extends SpecBase with BeforeAndAfterEach with MockitoSugar {
  ...
  "HasSchemeChangedRules Controller" - {
    ...
    "must save the answer and redirect to the next page when valid data is submitted (No)" in {
      ...
      running(application) {
        ...
        val result = route(application, request).value
        val updatedAnswers = emptyUserAnswers.set(HasSchemeChangedRulesPage, false).success.value
