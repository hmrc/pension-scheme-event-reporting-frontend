...
class DeclarationPspControllerSpec extends SpecBase with BeforeAndAfterEach {
  ...
  "DeclarationPsp Controller" - {
    ...
    "must save the answer and redirect to the next page when valid data is submitted" in {
      ...
      when(mockEventReportingConnector.submitReportEvent20A(any(), any())(any(), any())).thenReturn(Future.successful())
      when(mockUserAnswersCacheConnector.save(any(), any(), any())(any(), any()))
        .thenReturn(Future.successful(()))
      ...
      running(application) {
        ...
        val result = route(application, request).value
        val updatedAnswers = emptyUserAnswers.set(DeclarationPspPage, validValue).success.value

        status(result) mustEqual SEE_OTHER
        redirectLocation(result).value mustEqual DeclarationPspPage.navigate(waypoints, emptyUserAnswers, updatedAnswers).url
