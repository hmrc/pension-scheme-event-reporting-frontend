  ...
  "WhatYouWillNeed Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        ...
        contentAsString(result) mustEqual view(controllers.event13.routes.SchemeStructureController.onPageLoad(EmptyWaypoints).url)(request, messages(application)).toString
      }
    }
  }
}
