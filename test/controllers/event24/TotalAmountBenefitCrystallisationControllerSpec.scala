...
import forms.event24.TotalAmountBenefitCrystallisationFormProvider
...
import views.html.event24.TotalAmountBenefitCrystallisationView
...
class TotalAmountBenefitCrystallisationControllerSpec extends SpecBase with BeforeAndAfterEach {
  ...
  private def getRoute: String = routes.TotalAmountBenefitCrystallisationController.onPageLoad(waypoints, 0).url
  private def postRoute: String = routes.TotalAmountBenefitCrystallisationController.onSubmit(waypoints, 0).url
  ...
  "TotalAmountBenefitCrystallisation Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers().set(TotalAmountBenefitCrystallisationPage(0), validValue).success.value
    ...
    "must save the answer and redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        val result = route(application, request).value
        val updatedAnswers = emptyUserAnswers.set(TotalAmountBenefitCrystallisationPage(0), validValue).success.value

        status(result) mustEqual SEE_OTHER
        redirectLocation(result).value mustEqual TotalAmountBenefitCrystallisationPage(0).navigate(waypoints, emptyUserAnswers, updatedAnswers).url
