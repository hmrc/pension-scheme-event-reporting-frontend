  ...
  "signOut" - {
    ...
    "must redirect to the continue URL and clear down session cache and UA cache" in {
      ...
      running(application) {
        ...
        val encodedExitSurveyUrl = URLEncoder.encode(appConfig.exitSurveyUrl, "UTF-8")
        val expectedRedirectUrl = s"${appConfig.signOutUrl}?continue=$encodedExitSurveyUrl"
  ...
  "signOutNoSurvey" - {
    "must redirect to sign out, specifying SignedOut as the continue URL and clear down session cache and UA cache" in {
      ...
      running(application) {
        ...
        val encodedContinueUrl = URLEncoder.encode(appConfig.signOutNoSurveyUrl, "UTF-8")
