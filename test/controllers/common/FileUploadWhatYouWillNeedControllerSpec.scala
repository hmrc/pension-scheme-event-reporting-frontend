...
class FileUploadWhatYouWillNeedControllerSpec extends SpecBase {
  ...
  private def getRoute: String = controllers.common.routes.FileUploadWhatYouWillNeedController.onPageLoad().url
  ...
  "FileUploadWhatYouWillNeed Controller" - {
    ...
    "must return OK and the correct view for a GET (Event 23)" in {
      ...
      running(application) {
        ...
        contentAsString(result) mustEqual view(continueUrl.url, templateDownloadLink, instructionsDownloadLink, Event23)(request, messages(application)).toString
      }
    }
  }
}
