  ...
  "WhatYouWillNeed Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        ...
        contentAsString(result).removeAllNonces()mustEqual view(controllers.event10.routes.BecomeOrCeaseSchemeController.onPageLoad(EmptyWaypoints).url)(request, messages(application)).toString
      }
    }
  }
}
