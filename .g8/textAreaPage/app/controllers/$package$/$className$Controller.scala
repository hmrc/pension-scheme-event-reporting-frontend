...
class $className$Controller @Inject()(val controllerComponents: MessagesControllerComponents,
  ...
  def onPageLoad(waypoints: Waypoints): Action[AnyContent] = (identify andThen getData(eventType)) { implicit request =>
    val preparedForm = request.userAnswers.flatMap(_.get($className$Page)).fold(form){v => form.fill(Some(v))}
      ...
      form.bindFromRequest().fold(
        ...
        value => {
          ...
          val updatedAnswers = value match {
            case Some(v) => originalUserAnswers.setOrException($className$Page, v)
            case None => originalUserAnswers.removeOrException($className$Page)
          }
