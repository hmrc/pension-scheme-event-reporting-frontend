...
class $className$ControllerSpec extends SpecBase with MockitoSugar {
  ...
  private val validAnswer = $className$("value 1", "value 2")
  private val userAnswers = emptyUserAnswers.set($className$Page, validAnswer).success.value
  ...
  "$className$ Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill($className$("value 1", "value 2")), waypoints)(request, messages(application)).toString
      }
    }
