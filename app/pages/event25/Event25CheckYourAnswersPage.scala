...
case class Event25CheckYourAnswersPage(index: Index) extends CheckAnswersPage {
  override val urlFragment: String =
    s"event-${Event25.toString}-check-answers-${index.display}"

  override def route(waypoints: Waypoints): Call = {
    controllers.event25.routes.Event25CheckYourAnswersController.onPageLoad(index)
  }
  ...
  def waypointFromString(s: String): Option[Waypoint] = {
    ...
    s match {
      case pattern(indexDisplay) =>
        Some(Event25CheckYourAnswersPage(Index(indexDisplay.toInt - 1)).waypoint)
      case _ =>
        None
    }
  }
