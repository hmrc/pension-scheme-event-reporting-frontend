...
object TypeOfProtection extends Enumerable.Implicits {
  ...
  case object PrimaryProtection extends WithName("primaryProtection") with TypeOfProtection
  ...
  val values: Seq[TypeOfProtection] = Seq(
    PrimaryProtection, EnhancedProtection
  )

  def options(implicit messages: Messages): Seq[RadioItem] = values.zipWithIndex.map {
    case (value, index) =>
      RadioItem(
        content = Text(messages(s"event8a.typeOfProtection.${value.toString}")),
