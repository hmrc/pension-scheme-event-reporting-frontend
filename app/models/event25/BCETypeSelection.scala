...
object BCETypeSelection extends Enumerable.Implicits {
  ...
  case object Commencement extends WithName("commencement") with BCETypeSelection
  ...
  def options(implicit messages: Messages): Seq[RadioItem] = values.zipWithIndex.map {
    case (value, index) =>
      RadioItem(
        content = Text(messages(s"bceTypeSelection.event25.${value.toString.replaceAll("[, ]", "")}")),
