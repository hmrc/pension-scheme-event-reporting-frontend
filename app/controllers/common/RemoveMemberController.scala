...
class RemoveMemberController @Inject()(
  ...
  private val eventType = EventType.Event1 // TODO: parameterise per event.
  ...
  def onPageLoad(waypoints: Waypoints): Action[AnyContent] = (identify andThen getData(eventType) andThen requireData) { implicit request =>
    val preparedForm = request.userAnswers.get(RemoveMemberPage).fold(form)(form.fill)
    ...
    implicit request =>
      form.bindFromRequest().fold(
        ...
        value => {
          ...
          val updatedAnswers = originalUserAnswers.setOrException(RemoveMemberPage, value)
          userAnswersCacheConnector.save(request.pstr, eventType, updatedAnswers).map { _ =>
          Redirect(RemoveMemberPage.navigate(waypoints, originalUserAnswers, updatedAnswers).route)
        }
      }
    )
  }
