...
class EmployerTangibleMoveablePropertyController @Inject()(val controllerComponents: MessagesControllerComponents,
                                                           identify: IdentifierAction,
                                                           getData: DataRetrievalAction,
                                                           userAnswersCacheConnector: UserAnswersCacheConnector,
                                                           formProvider: EmployerTangibleMoveablePropertyFormProvider,
                                                           view: EmployerTangibleMoveablePropertyView
  ...
  def onPageLoad(waypoints: Waypoints, index: Index): Action[AnyContent] = (identify andThen getData(eventType)) { implicit request =>
    val preparedForm = request.userAnswers.flatMap(_.get(EmployerTangibleMoveablePropertyPage(index))).fold(form) { v => form.fill(v) }
    ...
    implicit request =>
      form.bindFromRequest().fold(
        ...
        value => {
          ...
          val updatedAnswers = originalUserAnswers.setOrException(EmployerTangibleMoveablePropertyPage(index), value)
