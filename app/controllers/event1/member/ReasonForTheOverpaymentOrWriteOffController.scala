...
class ReasonForTheOverpaymentOrWriteOffController @Inject()(val controllerComponents: MessagesControllerComponents,
  ...
  def onPageLoad(waypoints: Waypoints, index: Index): Action[AnyContent] = (identify andThen getData(eventType)) { implicit request =>
    val preparedForm = request.userAnswers.flatMap(_.get(ReasonForTheOverpaymentOrWriteOffPage(index))).fold(form)(form.fill)
    ...
    implicit request =>
      form.bindFromRequest().fold(
        ...
        value => {
          ...
          val updatedAnswers = originalUserAnswers.setOrException(ReasonForTheOverpaymentOrWriteOffPage(index), value)
          userAnswersCacheConnector.save(request.pstr, eventType, updatedAnswers).map { _ =>
            Redirect(ReasonForTheOverpaymentOrWriteOffPage(index).navigate(waypoints, originalUserAnswers, updatedAnswers).route)
          }
        }
      )
  }
