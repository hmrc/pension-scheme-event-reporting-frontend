...
class ErrorDescriptionController @Inject()(val controllerComponents: MessagesControllerComponents,
                                         identify: IdentifierAction,
                                         getData: DataRetrievalAction,
                                         userAnswersCacheConnector: UserAnswersCacheConnector,
                                         formProvider: ErrorDescriptionFormProvider,
                                         view: ErrorDescriptionView
      ...
      form.bindFromRequest().fold(
        ...
        value => {
          ...
          userAnswersCacheConnector.save(request.pstr, eventType, updatedAnswers).map { _ =>
            Redirect(ErrorDescriptionPage.navigate(waypoints, originalUserAnswers, updatedAnswers).route)
          }
        }
      )
  }
