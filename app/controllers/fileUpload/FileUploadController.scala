...
import pages.fileUpload.{FileUploadPage, FileUploadResultPage}
...
class FileUploadController @Inject()(val controllerComponents: MessagesControllerComponents,
  ...
  def onPageLoad(waypoints: Waypoints, eventType: EventType): Action[AnyContent] = (identify andThen getData(eventType)
    ...
    val checkingRedirectUrl = appConfig.checkingEndPointTarget(eventType)
    val errorRedirectUrl = appConfig.failureEndPointTarget(eventType)
    upscanInitiateConnector.initiateV2(Some(checkingRedirectUrl), Some(errorRedirectUrl), eventType).map { uir =>
  ...
  def onSubmit(waypoints: Waypoints, eventType: EventType): Action[AnyContent] =  (identify andThen getData(eventType)) async {
    implicit request =>
      ...
      val isFileNotFoundErrorPresent = Try(request.queryString("errorMessage")) match {
        case Success(value) => value.contains("""'file' field not found""")
        case Failure(_) => false
      }
      if (isFileNotFoundErrorPresent) {
        val errorRedirectUrl = appConfig.failureEndPointTarget(eventType)
       // Future.successful(Redirect(errorRedirectUrl))
        ...
        request.queryString.get("key").flatMap(_.headOption) match {
          ...
          Future.successful(Ok(view(waypoints, getEventTypeByName(eventType), eventType, Call("post", errorRedirectUrl), formFields, noFileUploaded)))
