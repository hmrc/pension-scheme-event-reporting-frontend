...
import models.TaxYear.getSelectedTaxYearAsRangeOfStrings
...
class WhatYouWillNeedController @Inject()(
  ...
  def onPageLoad(waypoints: Waypoints): Action[AnyContent] = (identify andThen getData(eventType) andThen requireData) {
    implicit request =>
      val selectedTaxYear = getSelectedTaxYearAsRangeOfStrings(request.userAnswers)
      Ok(view(controllers.event14.routes.HowManySchemeMembersController.onPageLoad(waypoints).url, selectedTaxYear))
  }
}
