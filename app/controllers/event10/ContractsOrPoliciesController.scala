...
class ContractsOrPoliciesController @Inject()(
                                               val controllerComponents: MessagesControllerComponents,
                                               identify: IdentifierAction,
                                               getData: DataRetrievalAction,
                                               requireData: DataRequiredAction,
                                               userAnswersCacheConnector: UserAnswersCacheConnector,
                                               formProvider: ContractsOrPoliciesFormProvider,
                                               view: ContractsOrPoliciesView
  ...
  def onPageLoad(waypoints: Waypoints): Action[AnyContent] = (identify andThen getData(eventType) andThen requireData) { implicit request =>
    val preparedForm = request.userAnswers.get(ContractsOrPoliciesPage).fold(form)(form.fill)
    ...
    implicit request =>
      form.bindFromRequest().fold(
        ...
        value => {
          ...
          val updatedAnswers = originalUserAnswers.setOrException(ContractsOrPoliciesPage, value)
          userAnswersCacheConnector.save(request.pstr, eventType, updatedAnswers).map { _ =>
            Redirect(ContractsOrPoliciesPage.navigate(waypoints, originalUserAnswers, updatedAnswers).route)
          }
        }
      )
  }
