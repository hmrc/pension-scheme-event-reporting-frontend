...
import viewmodels.checkAnswers.{BecomeOrCeaseSchemeSummary, ContractsOrPoliciesSummary, SchemeChangeDateSummary}
...
class Event10CheckYourAnswersController @Inject()(
  ...
  def onClick: Action[AnyContent] =
    (identify andThen getData(Event10) andThen requireData) { implicit request => // TODO: reimplement async when compile is added in backend: `...().async`
      //      connector.compileEvent(request.pstr, Event10).map {
      //        _ =>
