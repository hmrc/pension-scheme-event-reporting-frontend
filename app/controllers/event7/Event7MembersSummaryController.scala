...
class Event7MembersSummaryController @Inject()(
  ...
  def onPageLoad(waypoints: Waypoints): Action[AnyContent] =
    (identify andThen getData(eventType) andThen requireData) { implicit request =>
      ...
      val mappedMembers = getMappedMembers(request.userAnswers, request.readOnly())
  ...
  private def getMappedMembers(userAnswers: UserAnswers, isReadOnly: Boolean)(implicit messages: Messages): Seq[SummaryListRowWithThreeValues] = {
