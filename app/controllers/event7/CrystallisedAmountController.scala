...
class CrystallisedAmountController @Inject()(val controllerComponents: MessagesControllerComponents,
                                        identify: IdentifierAction,
                                        getData: DataRetrievalAction,
                                        userAnswersCacheConnector: UserAnswersCacheConnector,
                                        formProvider: CrystallisedAmountFormProvider,
                                        view: CrystallisedAmountView
    ...
    implicit request =>
      form.bindFromRequest().fold(
        ...
        value => {
          ...
          val updatedAnswers = originalUserAnswers.setOrException(CrystallisedAmountPage(index), value)
          userAnswersCacheConnector.save(request.pstr, eventType, updatedAnswers).map { _ =>
            Redirect(CrystallisedAmountPage(index).navigate(waypoints, originalUserAnswers, updatedAnswers).route)
          }
        }
      )
  }
