...
import pages.event24.{BCETypeSelectionPage, CrystallisedDatePage, EmployerPayeReferencePage, Event24CheckYourAnswersPage, MarginalRatePage, OverAllowanceAndDeathBenefitPage, OverAllowancePage, TotalAmountBenefitCrystallisationPage, TypeOfProtectionReferencePage, ValidProtectionPage}
...
class UserAnswersValidation @Inject()(compileService: CompileService) {
  ...
  def event24AnswerValidation(index: Index)(implicit hc: HeaderCarrier, executor: ExecutionContext, request: DataRequest[AnyContent]): Future[Result] = {
